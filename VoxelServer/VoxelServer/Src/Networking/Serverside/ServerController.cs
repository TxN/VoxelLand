	...
	public class ServerController : ServerSideController<ServerController> {
		...
		public bool TryAuthenticate(ClientState client, string userName, string password, out bool newUser) {
			...
			if ( profile == null ) {
				...
				DebugOutput.Log(string.Format("Registering new user with name '{0}'", userName));
				newUser = true;
		...
		public void ForceDisconnectClient(ClientState client, string message) {
			...
			DebugOutput.LogFormat("Client with id {0} force disconnected with reason: '{1}'", client.ConnectionID, message);
		...
		void OnNewConnection(int connectionId) {
			...
			DebugOutput.LogFormat("Client with id {0} connecting. Sending handshake command.", connectionId);
		}
		...
		void OnClientDisconnect(int connectionId) {
			...
			DebugOutput.LogFormat("Player '{0}' with id '{1}' disconnected.", client.UserName, connectionId);
